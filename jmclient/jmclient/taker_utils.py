...
def direct_send(ws, amount, mixdepth, destaddr, answeryes=False,
    ...
    assert amount >=0
    assert isinstance(ws.wallet, BaseWallet)

    from pprint import pformat
    txtype = ws.get_txtype()
    if amount == 0:
        utxos = ws.get_utxos_by_mixdepth()[mixdepth]
        if utxos == {}:
            log.error(
        ...
        utxos = ws.select_utxos(mixdepth, amount + initial_fee_est)
        if len(utxos) < 8:
        ...
        change_addr = ws.get_internal_addr(mixdepth)
    ...
    txsigned = sign_tx(ws.wallet, mktx(list(utxos.keys()), outs), utxos)
